...
{
    tinyjpeg_idct(ic->Q_tables[ic->q[0]], idata->DCT_Y[0], yuvdata->Y, 16);
    tinyjpeg_idct(ic->Q_tables[ic->q[0]], idata->DCT_Y[1], yuvdata->Y + 8, 16);
    tinyjpeg_idct(ic->Q_tables[ic->q[0]], idata->DCT_Y[2], yuvdata->Y + 64 * 2, 16);
    tinyjpeg_idct(ic->Q_tables[ic->q[0]], idata->DCT_Y[3], yuvdata->Y + 64 * 2 + 8, 16);
    tinyjpeg_idct(ic->Q_tables[ic->q[1]], idata->DCT_C[0], yuvdata->Cb, 8);
    tinyjpeg_idct(ic->Q_tables[ic->q[2]], idata->DCT_C[1], yuvdata->Cr, 8);
}
struct idct_context *create_idct_context(struct jpeg_parse_context *jpc)
{
    ...
    memcpy(ic->Q_tables, jpc->Q_tables, sizeof(float) * 3 * 64);
    memcpy(ic->q, jpc->q, sizeof(int) * 3);
    ic->width = jpc->width;
