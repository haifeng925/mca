...
static int get_next_huffman_code(struct jdec_task *hdata, struct huffman_table *huffman_table)
{
    ...
    do
    {
        do
        {
            while (hdata->nbits_in_reservoir < (9))
            {
                unsigned char c;
                if (hdata->stream >= hdata->stream_end)
                    return - 1;
                c = *hdata->stream++;
                hdata->reservoir <<= 8;
                if (c == 0xff && *hdata->stream == 0x00)
                    hdata->stream++;
                hdata->reservoir |= c;
                hdata->nbits_in_reservoir += 8;
            }
        }
        while (0);
        ;
        hcode = ((hdata->reservoir) >> (hdata->nbits_in_reservoir - (9)));
    }
    while (0);
    ;
    ...
    if (value >= 0)
    {
        ...
        do
        {
            hdata->nbits_in_reservoir -= (code_size);
            hdata->reservoir &= ((1U << hdata->nbits_in_reservoir) - 1);
        }
        while (0);
        ;
        return value;
    ...
    for (extra_nbits = 0;
        ...
        extra_nbits++)
    {
        nbits = 9 + 1 + extra_nbits;
        do
        {
            do
            {
                while (hdata->nbits_in_reservoir < (nbits))
                {
                    unsigned char c;
                    if (hdata->stream >= hdata->stream_end)
                        return - 1;
            ...
            while (0);
            ;
            hcode = ((hdata->reservoir) >> (hdata->nbits_in_reservoir - (nbits)));
        }
        while (0);
        ;
        ...
        while (slowtable[0])
        {
            if (slowtable[0] == hcode)
            {
                do
                {
                    hdata->nbits_in_reservoir -= (nbits);
                    hdata->reservoir &= ((1U << hdata->nbits_in_reservoir) - 1);
                }
                while (0);
                ;
...
static int process_Huffman_data_unit(struct huffman_context *hc, struct jdec_task *hdata, int component, short int *DCT_out)
{
    ...
    if (huff_code)
    {
        do
        {
            do
            {
                while (hdata->nbits_in_reservoir < (huff_code))
                {
                    unsigned char c;
                    if (hdata->stream >= hdata->stream_end)
                        return - 1;
            ...
            while (0);
            ;
            DCT[0] = ((hdata->reservoir) >> (hdata->nbits_in_reservoir - (huff_code)));
            hdata->nbits_in_reservoir -= (huff_code);
            hdata->reservoir &= ((1U << hdata->nbits_in_reservoir) - 1);
    ...
    while (j < 64)
    {
        ...
        else
        {
            ...
            do
            {
                do
                {
                    while (hdata->nbits_in_reservoir < (size_val))
                    {
                        unsigned char c;
                        if (hdata->stream >= hdata->stream_end)
                            return - 1;
                ...
                while (0);
                ;
                DCT[j] = ((hdata->reservoir) >> (hdata->nbits_in_reservoir - (size_val)));
                hdata->nbits_in_reservoir -= (size_val);
                hdata->reservoir &= ((1U << hdata->nbits_in_reservoir) - 1);
...
struct huffman_context *create_huffman_context(struct jpeg_parse_context *jpc)
{
    ...
    memcpy(hc->component_infos, jpc->component_infos, sizeof(struct component) * 3);
