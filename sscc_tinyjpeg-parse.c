...
static void print_SOF(const unsigned char *stream)
{
    ...
    precision = stream[2];
    height = (((stream + 3)[0] << 8) | (stream + 3)[1]);
    width = (((stream + 5)[0] << 8) | (stream + 5)[1]);
...
static void build_huffman_table(const unsigned char *bits, const unsigned char *vals, struct huffman_table *table)
{
    ...
    unsigned char huffsize[256 + 1], *hz;
    unsigned int huffcode[256 + 1], *hc;
    int next_free_entry;
    hz = huffsize;
    for (i = 1;
        i <= 16;
    ...
    *hz = 0;
        ...
        i++)
    {
        ...
        {
            uint16_t *slowtable = table->slowtable[code_size - 9 - 1];
            while (slowtable[0])
                slowtable += 2;
            slowtable[0] = code;
            slowtable[1] = val;
...
{
    if ((jpc->flags & (1 << 1)) && jpc->default_huffman_table_initialized)
        return;
...
static int parse_DQT(struct jpeg_parse_context *jpc, const unsigned char *stream)
{
    ...
    while (0);
    dqt_block_end = stream + (((stream)[0] << 8) | (stream)[1]);
    stream += 2;
...
static int parse_DHT(struct jpeg_parse_context *jpc, const unsigned char *stream)
{
    ...
    while (length > 0)
    {
        ...
        if (count >= 256)
            do
            {
                snprintf(error_string, sizeof (error_string), "No more than %d bytes is allowed to describe a huffman table", 256);
                return - 1;
            }
            while (0);
        if ((index & 0xf) >= 4)
...
static int parse_DRI(struct jpeg_parse_context *jpc, const unsigned char *stream)
{
    ...
    if (length != 4)
        ...
        while (0);
    jpc->restart_interval = (((stream + 2)[0] << 8) | (stream + 2)[1]);
    do
    {
    }
    while (0);
...
int parse_JFIF(struct jpeg_parse_context *jpc, const unsigned char *stream)
{
    ...
    if ((jpc->component_infos[0].Hfactor < jpc->component_infos[1].Hfactor) || (jpc->component_infos[0].Hfactor < jpc->component_infos[2].Hfactor))
        do
        {
        ...
        while (0);
    if ((jpc->component_infos[0].Vfactor < jpc->component_infos[1].Vfactor) || (jpc->component_infos[0].Vfactor < jpc->component_infos[2].Vfactor))
        do
        {
        ...
        while (0);
    if ((jpc->component_infos[1].Hfactor != 1) || (jpc->component_infos[2].Hfactor != 1) || (jpc->component_infos[1].Vfactor != 1) || (jpc->component_infos[2].Vfactor != 1))
        do
        {
